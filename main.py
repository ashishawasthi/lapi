...
def epics(requirements: str):
    ...
    print("Calling OpenAI ChatCompletion API with messages: {messages}".format(messages=messages))
    epics_str = chat(messages).content
    print("Received epics from OpenAI ChatCompletion API: {epics}".format(epics=epics_str))
    epics_json = json.loads(epics_str)
    print("{epics_length} epics received from OpenAI ChatCompletion API".format(epics_length=len(epics_json)))
    return epics_json